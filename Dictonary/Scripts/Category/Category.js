...
//FACTORY
newsportalApp.factory('Category', ['$resource', function ($resource) {
    return $resource('', {}, {
        query: {
            method: 'GET',
        ...
        firstQuery: {
            method: 'GET',
            url: '/Home/GetFirstTimeCategoryData',
            params: { format: 'json' },
            isArray: false
        },
        sectionQuery: {
...
newsportalApp.controller('NewsCtrl', ['$scope', '$window', '$location', 'Category',
    function ($scope, $window, $location, Category) {
        ...
        if (urlPath !== "" || urlPath !== null) {
            ...
            categoryId = queryData[queryData.length - 2];
            $scope.categ = queryData[queryData.length - 1];
        }
        $scope.dataLoad = function () {
            Category.firstQuery({ categoryId: categoryId }).$promise.then(function (data) {
                if (data !== null) {
                    var totcount = parseInt(JSON.parse(data.paging)[0]["TotalCount"]);
                    $scope.page = 0;
                    $scope.pagesCount = parseInt(Math.ceil(totcount / 10));
                    $scope.totalCount = JSON.parse(data.paging)[0]["TotalCount"];
                    $scope.Data = {};
                    $scope.Data = JSON.parse(data.category);
                    $scope.btnHide('Popular', JSON.parse(data.popular));
                    $scope.btnHide('Latest', JSON.parse(data.latest));
                    $scope.isSearching = false;
                    $scope.title = $scope.Data[0].NewsType;
                    $scope.desc = $scope.Data[0].Description;
                    $("meta[description]").attr("content", $scope.desc);
                    loaderHide();
                }
        ...
        $scope.LoadMore = function (section) {
            ...
            if (section === 'Latest') {
                ...
                Category.sectionQuery({
                    categoryId: 0, pageIndex: $scope.latestPage, section: 'Latest'
        ...
        $scope.search = function (page) {
            ...
            Category.query({ page: page, pageSize: 10, categoryId: categoryId }).$promise.then(function (data) {
                var totcount = parseInt(JSON.parse(data.totalcount)[0]["totalCount"]);
